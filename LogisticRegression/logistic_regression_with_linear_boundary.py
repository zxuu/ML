...
iris_types = ['SETOSA','VERSICOLOR','VIRGINICA']   #三种花
...
x_train = data[[x_axis,y_axis]].values.reshape((num_examples,2))    #150*2
y_train = data['class'].values.reshape((num_examples,1))     #150*1

max_iterations = 1000    #迭代次数
polynomial_degree = 0    #不做特征变换
sinusoid_degree = 0    #不做特征变换
...
y_train_prections = logistic_regression.predict(x_train)    #预测值(花名)
'''
:[花的名字，花的名字，....].T
'''
...
samples= 150 
X = np.linspace(x_min,x_max,samples)    #在线性空间中以均匀步长生成数字序列
